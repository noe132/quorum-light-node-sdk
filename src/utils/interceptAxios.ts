...
export const interceptAxios = () => {
  ...
  axios.interceptors.response.use((response) => {
    try {
      const groupId = response.config.url!.match(UUID_REGEX)![0];
      if (groupId) {
        ...
        if (group) {
          ...
          if (group.chainAPIs.length > 0 && loopIndex > 0) {
            ...
            console.log(`Use alternative chain api ${group.chainAPIs[0]}`)
          }
        }
      }
    } catch (err) {
    ...
    const groupId = error.config.url.match(UUID_REGEX)[0];
    if (error.code === 'ERR_NETWORK' && groupId) {
      const group = cache.Group.get(groupId)!;
      ...
      if (curIndex <= chainAPIs.length - 1) {
        ...
        console.log(`Try alternative chain api ${chainAPIs[curIndex]}`)
